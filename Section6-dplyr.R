...
###Lecture 1. Filter commands in Dplyr
library(tidyverse)
data("mtcars")
head(mtcars)
...
### Lecture 3. Using rename function of Dplyr in R
library(tidyverse)  #OR: library(dplyr)
names(iris)
...
### Lecture 4. Using mutate() function of Dplyr in R
library(tidyverse)  #OR: library(dplyr)
...
### Lecture 5. Use select function in Dplyr to Choose Variables/Columns
library(tidyverse)
...
### Lecture 6. Join datasets with Dplyr in R
library(tidyverse)
...
### Lecture 7. slice function in dplyr
library(tidyverse)
...
### Lecture 8. summarise function in dplyr
...
### Lecture 9. gather() function in dplyr to convert multiple columns into one
library(tidyverse)
...
### Lecture 10. Use Spread() of Dplyr in R to Consolidate Multiple Rows into One
library(tidyverse)
...
### Lecture 11. Use separate() of Dplyr in R to divide a Single Column 
...
### Lecture 12. Use n(),Nth,n_distinct() of dplyr for counting in R
library(tidyverse)
#n() for basic counts
new.sleep <- msleep %>%
  group_by(vore, order)
...
### Lecture 13. Use na_if() and coalesce() of dplyr to deal with NA values
...
### Lecture 14. Ranking functions in dplyr
library(tidyverse)  #OR library(dplyr)
...
### Lecture 15. Random sampling with dplyr 
library(tidyverse)  #OR library(dplyr)
data("mtcars")
str(mtcars)
...
### Lecture 16. Count Observations with Dplyr
library(tidyverse)
...
### Lecture 17. case_when() with dplyr

library(tidyverse)

#Create a new variable after executing  the case when statement 
#and using mutate function:
#creating a data frame that holds car brands, names, prices, taxes. 
#Then using mutate() function, we are creating an additional column 
#(Price_level) that will store string literals: high, average, and 
#low after evaluating the price used inside the case_when() function.

# Creating a new variable using case_when() and mutate() function

data_price = data.frame(Brand=c("BMW", "Toyota",
                                "Volvo", "Mahindra", "Maruti Suzuki"),
...
data_price %>% mutate(Price_level=case_when(Price >= 500000 
                & Price <= 900000 ~ "Average", 
                Price > 900000 ~ "High", 
                TRUE ~ "Low"))
...
#creating a data frame that holds car brands, names, prices, taxes.
#Using mutate() function, we are creating an additional column 
#(Price_level) that will store string literals: high, average, 
#and low after evaluating the price used inside the case_when()
#function. Note that for cars having the price equal to NA 
#we are adding “NIL”, at the corresponding position of the 
#Price_level column

data_price = data.frame(Brand=c("Maruti Suzuki", "Tata Motors",
...
data_price %>% mutate(Price_level=case_when(is.na(Price) ~ "NIL", 
...
#checking whether the current value in the vector is divisible by 4,
#and if it is so then we are he replacing the multiples of 4 with 
#the string “Yes”.  

# Creating a vector
vector_test <- seq(2, 20, by = 2)
vector_test
...
### Lecture 18. Group by function in R using Dplyr


#Group_by() function alone will not give any output. It should be 
#followed by summarise() function with an appropriate action to
#perform. 
...
mpg_cylgear = mtcars %>% group_by(cyl, gear)  %>%
  summarise(meanmpg = mean(mpg))
...
### Lecture 19. Apply a function to each group using Dplyr in R


#group_by and mutate
#The following code snippet illustrates the procedure where 
#the data in the tibble is divided into groups based on the values 
#in col x. The sum of the y col values is then calculated and 
#returned group-wise. 
...
#group_by and group_map method
#The group_map() method can also apply a function to each group 
#in the tibble. The method returns the number of tibbles equivalent
#to the number of groups returned. 

#The following code snippet illustrates the usage of the 
#group_map() method on a grouped tibble, wherein user-defined 
#entries are grouped based on column x values. The sum of column y 
#values is computed against each tibble group value.
...
# Creating a tibble
data = tibble(
  x = c(12,14,15,16,12,15,16,6,12,16), 
  y = 1:10)
print("Data")
print(data)
...
data %>%
  group_by(x) %>%
  group_map(~sum_y(.x$y))      #.x for a grouped tibble , ~sum_y for a userwritten func
